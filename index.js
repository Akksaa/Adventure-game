...
let userDamage = Math.floor(Math.random() * 50) + 5;
let enemyDamage = Math.floor(Math.random() * 50) + 5;
let potion = 3;
let potionedHealth = Math.floor(Math.random() * 25 + 5);
const enemies = ["Zombie", "Skeleton", "Goblin", "Giant Spider"];
...
while (true) {
    ...
    if (whatTodo.option == "Attack") {
        let takenDamage = Math.floor(Math.random() * userDamage) + 1;
        let givenDamage = Math.floor(Math.random() * enemyDamage) + 1;
